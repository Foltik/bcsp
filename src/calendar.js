...
function Day(props) {
    const items = props.items.map(item => {
        ...
        return <div class="item" style={{top: percentage(pos), height: percentage(height)}}>{item.name}</div>;
    });
...
function Days(props) {
    return (
        <div className="days">
            {[...Array(5)].map((e, i) =>
                 <Day start={props.start} len={props.len} items={props.items[i]} />)
            }
        </div>
    );
}
    ...
    return (
        <div className="times">
            {[...Array(Math.floor(props.len / 60) + 1)].map((e, i) =>
                <Time time={props.start + (i * 60)} pos={(i * 60) / props.len} />
            )}
        </div>
    );
}
